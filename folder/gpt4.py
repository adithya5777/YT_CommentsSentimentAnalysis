...
# Filter comments with specific emotions
selected_emotions = ["joy", "anger", "negative", "positive", "disgust"]
filtered_comments = df[df["Emotion"].isin(selected_emotions)]

# Store filtered comments in separate CSV files
for emotion in selected_emotions:
    filename = f"{emotion}_comments.csv"
    filtered_emotion_comments = filtered_comments[filtered_comments["Emotion"] == emotion]
    filtered_emotion_comments.to_csv(filename, index=False)
